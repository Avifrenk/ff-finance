...
- [ ] **Фаза 8.3. Мобильная адаптация: safe area, навбар, кнопка +.**
  ...
  - `index.css`: убрать `prefers-color-scheme: dark` (UI ориентирован на светлый, тёплый, как и манифест). Reasoning: жена пользуется днём, ночью телефон в режиме «не беспокоить» → пуш-уведомления, не приложение. Поддерживать второй theme = двойная работа без пользы. Если когда-то понадобится — отдельная ветка.
